...
def process_log_data(which_log):
    ...
    for i, log_path in enumerate(log_folders):
        results_path = f'{log_path}/test_performance/testing_record_in_training.csv'
...
# Define log paths for the algorithms
logs = {
    'ES-RL': './...',
    'SPN-CWS': './...',
    'GAT': './...'
...
plt.title('Convergence on testing')
plt.legend()
plt.grid(True)

xticks = ax.get_xticks()
ax.set_xticklabels([f'{int(x*20)}' for x in xticks])

output_dir = '../'  # Replace this with the actual save path
if not os.path.exists(output_dir):
    os.makedirs(output_dir)
output_file = os.path.join(output_dir, 'convergence_plot_testing.pdf')
