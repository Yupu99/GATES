...
class ESOpenAI(BaseOptim):
    ...
    def init_population(self, policy: torch.nn.Module, env):
        ...
        self.mu_model_flatten_params = torch.tensor(get_flatten_params(self.mu_model)['params'], requires_grad=True, dtype=torch.float32)
        print('Number of paras ES will updated:'.format(), self.mu_model_flatten_params.shape)
    ...
    def next_population(self, assemble, results, g):
        ...
        self.mu_model_flatten_params.grad = torch.tensor(grad_param_list, dtype=torch.float32)
        self.optimizer.step()
